...
describe('composeCoveragePercent', () => {
  ...
  it('least-effort clamp at top: A100 + mA0.7 → 100 then sync knobs', () => {
    const v = composeCoveragePercent(100, 0.7);
    expect(v).toBe(HIGH_COVERAGE_MAX);
    expect(decomposeCoveragePercent(v)).toEqual({ a: 100, mA: 0 });
  });
  ...
  it('compose then normalize stays in high-coverage band', () => {
    for (let a = COVERAGE_A_MIN; a <= COVERAGE_A_MAX; a += 10) {
      for (let t = 0; t <= 9; t++) {
        ...
        const v = composeCoveragePercent(a, mA);
