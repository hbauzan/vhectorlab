...
### 4.5. Control Espacial 3D — defaults = mid del rango + steps finos
- **Problema**: Rangos históricos asimétricos (Distancia Y / Grosor con default = min; Amplitud hasta 240) dejan el thumb pegado a un extremo al load — solo se puede agrandar, no afinar alrededor del punto dulce. Steps gruesos (enteros / 0.1 en rangos chicos) impiden valores intermedios.
- **Solución Obligatoria**: Un solo set global de min/max simétrico lineal (`mid === (min+max)/2` dentro del `step`) con defaults fijos: Separación $X=0.4$ ∈ $[0.1,0.7]$ step $0.05$, Distancia $Y=10$ ∈ $[1,19]$ step $0.1$, Amplitud $Y=7$ ∈ $[1,13]$ step $0.1$, Longitud $Z=0.2$ ∈ $[0.1,0.3]$ step $0.01$, Grosor $=0.10$ ∈ $[0.05,0.15]$ step $0.01$. Labels: 2 dec (X/Z/Grosor), 1 dec (Y). Mismos rangos en todos los MODE/VISTA/RENDER (presets por vista = futuro).
- **Invariante**: al load, cada thumb del dock derecho arranca en el centro del track; se puede disminuir y aumentar con granularidad gradual.
