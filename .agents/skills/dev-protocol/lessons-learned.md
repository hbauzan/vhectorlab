...
- **Collapse / dock stack**: Visualization vuelve **bajo Spatial Controls** en el right dock. Tab estilo `dock-tab` en la **arriba-derecha** del card (gutter 28px dentro de host 280; body glass 252 + padding/gap compactos). Glifos ▼/▲ (colapso vertical). No tab izquierda (peleaba con el ▶ del dock). No pegar al HUD. Key `vl3d.viz.panelCollapsed`. Chrome en `.viz-panel-body`.
