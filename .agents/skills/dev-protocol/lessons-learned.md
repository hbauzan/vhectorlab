...
- **Collapse / dock stack**: Visualization bajo Spatial Controls. Handle **superior** (franja horizontal `dock-tab`, ▼/▲) unida al card; body más compacto (260px, gaps/padding apretados). No tab lateral. HUD: `clamp`/`vw` side-to-side (desktop deja aire al gizmo; mobile safe-area full width). Key `vl3d.viz.panelCollapsed`.
- **Mobile chrome density**: navbar tabs ~28px / ~0.62rem; HUD strip compacta y fluida al ancho — no forzar 44px en chrome decorativo (inputs/CTAs del panel sí mantienen 44px / 16px iOS zoom guard).
