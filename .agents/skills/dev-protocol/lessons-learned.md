...
- **Collapse tab**: Visualization collapse is **vertical** (▲/▼) — never a left side-tab inside the right dock. Side ◀/▶ inflated host width past Spatial Controls; with `.dock-right .dock-body { align-items: flex-end }` the handle jumped and fought the dock ▶ tab. Desktop: handle strip above the card, host width locked to 280px. Phone: bottom sheet on app root (`resolveVisualizationMountParent`); `data-viz-layout="sheet"`. Key `vl3d.viz.panelCollapsed`. **Chrome del card va en `.viz-panel-body`**. Touch blocklist incluye `#visualization-controls-container`.
