...
- **Collapse / HUD glue**: Visualization **siempre** monta en app root (nunca dentro del right dock). Expanded: sheet encima del HUD (`bottom: inset+hudHeight`), abre hacia arriba. Collapsed: píldora `48×48` en el slot derecho, misma baseline que el HUD (no flota). HUD inset (`left/right` no edge-to-edge) deja hueco para la píldora. Glifos ▲/▼. Key `vl3d.viz.panelCollapsed`. Chrome en `.viz-panel-body`. Touch blocklist: `#visualization-controls-container`.
